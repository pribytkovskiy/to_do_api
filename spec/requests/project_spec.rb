RSpec.describe 'Projects requests', type: :request do
  ...
  let(:token) { { Authorization: JsonWebToken.encode({ user_id: user.id }), accept: 'application/json' } }
    ...
    context 'logged in user' do
      context 'with valid params' do
        it 'creates project and returns projects data, corresponding to shema with status created', :show_in_doc do
          post api_v1_projects_path, params: valid_params, headers: devise_token
          expect(response).to have_http_status(:created)
          expect(response).to match_response_schema('projects/project')
        end

        it 'creates new project record in db' do
          expect { post api_v1_projects_path, params: valid_params, headers: devise_token }.to(
      ...
      context 'with invalid params' do
        it 'returns errors object with status unprocessable entity', :show_in_doc do
          post api_v1_projects_path, params: invalid_params, headers: devise_token
  ...
  describe 'GET /api/v1/projects/:id' do
    context 'logged in user' do
      it 'creates project and returns projects data, corresponding to shema with status created', :show_in_doc do
        get api_v1_project_path(project), headers: devise_token
        expect(response).to have_http_status(200)
        expect(response).to match_response_schema('projects/project')
      end
    end
    ...
    context 'logged in user' do
      context 'with valid params' do
        it 'updates project record' do
          ...
          expect { patch api_v1_project_path(project), params: valid_update_params, headers: devise_token }.to(
        ...
        it 'returns project with updated data with status ok', :show_in_doc do
          patch api_v1_project_path(project), params: valid_update_params, headers: devise_token
          expect(response).to have_http_status(:created)
          expect(response).to match_response_schema('projects/project')
        end
      end
      ...
      context 'with invalid params' do
        ...
        it 'returns errors object with status unprocessable entity', :show_in_doc do
          patch api_v1_project_path(project), params: invalid_params, headers: devise_token
          expect(response).to have_http_status(:unprocessable_entity)
        end
      end
    end
  ...
  describe 'DELETE /api/v1/projects/:id' do
    context 'logged in user' do
      it 'deletes project record from db', :show_in_doc do
        project
        expect { delete api_v1_project_path(project), headers: devise_token }.to(
          change { Project.count }.from(1).to(0)
        )
      end

      it 'returns nothing', :show_in_doc do
        delete api_v1_project_path(project), headers: devise_token
